<template>
	<view>
		
				<u-navbar
				           title="今日天气"
				           :autoBack="false"
						   :fixed="false"
						   :border="true"
						  :safeAreaInsetTop="false"
							@leftClick="$emit('switchMain',1)"
				       >
				       </u-navbar>
				<view>
					<view class="list-item wh-100" >
						<view style="padding: 1rem;">
							<view  >
								<u--image radius="15px 15px 0 0" :showLoading="true" src="https://img-qn.51miz.com/preview/element/00/01/27/38/E-1273800-6287C2E4.jpg" width="100%" height="120px" :lazy-load="true"  mode="aspectFill" ></u--image>
							</view>
							<view  class="weather-card">
								<view >
									<u-row>
										<u-col span="6" align="start">
											<u-icon
											style="z-index: 100;"
											:bold="true"
											@click="show=true" 
											    name="arrow-down"
											    size="16"
												:label="wheather.cityName"
												labelPos="left"
												color="#fff"
												labelColor="#fff"
											></u-icon>
										</u-col>
										<u-col span="6" >
											<u--text :text="todayDate" align="right" color="#fff"></u--text>
										</u-col>
									</u-row>
								</view>
								
								<view>
									<u-row justify="space-between" align="center">
										<u-col span="6" align="center">
											<view class="flex">
												<view >
													<u-icon :name="$url+wheather.weatherIcon" :label="wheather.weather" 
												  size="24" color="#fff" labelColor="#fff"style="filter: brightness(10);"></u-icon>
												</view>
											</view>
											<u--text :text="wheather.minTemperature+'℃~'+wheather.maxTemperature+'℃'"   color="#fff" size="20" align="center"></u--text>
										</u-col>
										<u-col span="6" align="center">
											<u--text :text="wheather.currentTemperature+'℃'"  color="#fff" size="28" align="center" ></u--text>
										</u-col>
									</u-row>
									<view>
										<br>
										<view>
											<u-row justify="space-between">
												<u-col span="3"  align="center">
													<u--text text="湿度" align="center" :bold="true"  color="#fff"></u--text>
													<text>{{wheather.humidity}}</text>
												</u-col>
												<u-col span="3"  align="center">
													<u--text text="风速" align="center" :bold="true"  color="#fff"></u--text>
													<text>{{wheather.windSpeed}} <text style="font-size: 12px;">km/h</text></text>
												</u-col>
												<u-col span="3"  align="center">
													<u--text text="能见度" align="center" :bold="true" color="#fff"></u--text>
													<text>{{wheather.visibility}} <text style="font-size: 12px;">%</text></text>
												</u-col>
												<u-col span="3"  align="center">
													<u--text text="空气质量" align="center" :bold="true" color="#fff"></u--text>
													<text>{{wheather.airQuality}}  <text style="font-size: 12px;">m</text></text>
												</u-col>
											</u-row>
										</view>
									</view>
								</view>	
							</view>
											
						</view>
					</view>
				</view>
			</view>
		
</template>

<script>
	export default {
		data() {
			return {
				wheather:[],
				
			}
		},
		mounted() {
			this.getWeather(13);
		},
		computed:{
			todayDate(){
				let date = new Date();
				let year = date.getFullYear();
				let month = date.getMonth() + 1; // 注意月份是从0开始计数的，所以需要+1
				let day = date.getDate();
				let formattedDate = `${year}-${month < 10 ? '0' + month : month}-${day < 10 ? '0' + day : day}`;
				return formattedDate;
			}
		},
		methods: {
			getWeather(cityId){
				uni.$u.http.get(`/control/weather/list`,{params:{cityId:cityId}})
				.then(res=>{
					console.log('结果是')
					console.log(res);
					if(res.data.code==200){
						this.wheather=res.data.rows[0]||this.wheather;
					}else{
						this.$request.requestShowToastError(res.msg);
					}
				})
				.catch(err=>{
					this.$request.requestShowToastError(err);
				})
			},
		}
	}
</script>

<style lang="scss">
	
	.list-item{
		position: relative;
		
		background-color: #fff;
	}
	.weather-card{
		padding: 1rem;
		background-color: #4690F9;
		box-shadow: 0px 0px 20px rgba(0, 0, 0, .5);
		color: #fff;
	}
</style>
